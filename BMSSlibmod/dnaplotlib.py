...
def write_label (ax, label_text, x_pos, opts=None):
    ...
    ax.text(x_pos+label_x_offset, label_y_offset+y_offset, label_text, horizontalalignment='center',
            ...
            color=label_color, rotation=label_rotation, zorder=30+zorder_add, fontweight='bold')
...
#    l1 = Line2D([start+(x_extent/2.0),start+(x_extent/2.0)],[0,y_extent], linewidth=linewidth,
#                color=color, zorder=8+zorder_add)
#
#    ax.add_line(l1)
...
def regulation (ax, type, num, from_part, to_part, scale, linewidth, arc_height_index, opts):
    ...
    arcHeightEnd = arcHeightStart*1 #*1.5
    ...
    if opts != None:
        ...
        if 'first_arc_y_offset' in list(opts.keys()):
            first_arc_y_offset = opts['first_arc_y_offset']
            arcHeightSpacing = arcHeightSpacing + first_arc_y_offset
            arcHeight = arcHeightConst + arc_height_index*arcHeightSpacing
        if 'second_arc_y_offset' in list(opts.keys()):
            second_arc_y_offset = opts['second_arc_y_offset']
            arcHeightConst = arcHeightConst + second_arc_y_offset
            arcHeightStart = arcHeightStart + second_arc_y_offset
            arcHeight = arcHeightConst + arc_height_index*arcHeightSpacing
            arcHeightEnd = arcHeightStart*1
...
class DNARenderer:
    ...
    def __init__(self, scale=1.0, linewidth=1.5, linecolor=(0,0,0),
